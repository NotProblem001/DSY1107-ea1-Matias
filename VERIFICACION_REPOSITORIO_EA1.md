...
| **2** | **Lectura con Permisos (Lector)** | `GET /pedidos` | Token de `lector@pedidos360.com` (`solicitudes/read`) | **`200 OK`** | API Gateway valida la firma del JWT y confirma la presencia del scope de lectura. Reenvía la petición al backend en ECS Fargate. |
| **3** | **Creación con Permisos (Cliente)** | `POST /pedidos` | Token de `cliente@pedidos360.com` (`solicitudes/write`) | **`201 Created`** | API Gateway verifica el scope `write`. Spring Boot persiste el pedido en la base de datos cloud y retorna el recurso creado. |
| **4** | **Aprobación con Permisos (Editor)** | `PUT /pedidos/1` | Token de `editor@pedidos360.com` (`solicitudes/approve`) | **`200 OK`** | API Gateway verifica el scope `approve`. Spring Boot actualiza el estado del pedido. |
...
| **6** | **Intento Ilegal de Escritura (Lector)** | `POST /pedidos` | Token de `lector@pedidos360.com` (Solo `solicitudes/read`) | **`403 Forbidden`** | **Prueba Reina del RA1:** El usuario está autenticado válidamente, pero API Gateway Scope Guard bloquea la petición en el perímetro porque el Access Token carece de `write`. Retorna `{"message":"Forbidden"}` sin consumir cómputo en el backend. |
| **7** | **Intento Ilegal de Creación (Editor)** | `POST /pedidos` | Token de `editor@pedidos360.com` (Solo `read` y `approve`) | **`403 Forbidden`** | API Gateway Scope Guard bloquea en el perímetro por falta de `write`. |
